    ...
    RefCountedPtr<LevelData<FArrayBox>> a_cCoef, const RealVect &a_dx)
{
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            {
                rhs_box(iv, c_U) =
                    -0.25 * (di_Vi[0][0] + di_Vi[1][1] + multigrid_vars_box(iv, c_V2_0) / yy );
            ...
            if (m_method_params.deactivate_zero_mode)
            {
                rhs_box(iv, c_V1) += -laplacian_V[0] - (di_Vi[0][cartoon_idx] / yy);
                rhs_box(iv, c_V2) += -laplacian_V[1] -(di_Vi[1][cartoon_idx] / yy) 
