...
GRSolver<method_t, matter_t>::GRSolver(GRParmParse &a_pp)
    : pp(a_pp), params(pp), numLevels(params.grid_params.numLevels),
      ...
      rhs(numLevels, NULL), aCoef(numLevels), bCoef(numLevels), cCoef(numLevels),
      diagnostic_vars(numLevels, NULL), Ham_error(0.), Mom_error(0.)
{
    metric = new Metric(params.metric_params);
    matter =
        new matter_t(params.matter_params, metric, params.grid_params.center,
                     params.grid_params.domainLength);
    method = new method_t(
        params.method_params, matter, metric, params.grid_params.numLevels,
        params.grid_params.center, params.base_params.G_Newton);
    ...
    diagnostics = new Diagnostics<method_t, matter_t>(
        method, matter, metric, params.base_params.G_Newton,
...
void GRSolver<method_t, matter_t>::setup()
{
    ...
    grids->define_operator(mlOp, aCoef, bCoef, cCoef, params.base_params.alpha,
    ...
    for (int NL_iter = 0; NL_iter < params.base_params.max_NL_iter; NL_iter++)
    {
        ...
        for (int ilev = 0; ilev < numLevels; ilev++)
        {
            ...
            method->set_elliptic_terms(multigrid_vars[ilev], rhs[ilev],
                                       aCoef[ilev], bCoef[ilev], cCoef[ilev],
                                       grids->vectDx[ilev]);
        }
        ...
        if ((Ham_error < params.base_params.HamMom_tolerance &&
             Mom_error < params.base_params.HamMom_tolerance))
        {
            break;
        }

        if (params.base_params.write_iter)
        {
...
void GRSolver<method_t, matter_t>::calculate_diagnostics(const int NL_iter)
{
    ...
    pout() << "The relative error of Mom before step " << NL_iter << " is "
           << Mom_error << " %" << endl;
    writeFile(params.base_params.error_filename, Ham_error, Mom_error);
}
...
void GRSolver<method_t, matter_t>::create_vars()
{
    ...
    for (int ilev = 0; ilev < numLevels; ilev++)
    {
        ...
        method->initialise_constraint_vars(*cCoef[ilev], dxLevel);
