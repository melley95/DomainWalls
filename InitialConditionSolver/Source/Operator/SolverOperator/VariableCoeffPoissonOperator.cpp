    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        This_will_not_compile !
            ...
            (CHF_FRA(a_lhs[dit]), CHF_CONST_FRA(dpsi[dit]),
             ...
             CHF_CONST_FRA((*m_bCoef)[dit]), CHF_CONST_FRA((*m_cCoef)[dit]),
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        This_will_not_compile !
            ...
            (CHF_FRA(a_lhs[dit]), CHF_CONST_FRA(dpsi[dit]),
             ...
             CHF_CONST_FRA((*m_cCoef)[dit]), CHF_BOX(region),
             CHF_CONST_REAL(m_dx));
    } // end loop over boxes
}
    ...
    for (DataIterator dit = a_dpsiFine.dataIterator(); dit.ok(); ++dit)
    {
        ...
        This_will_not_compile !
            ...
            (CHF_FRA_SHIFT(res, civ), CHF_CONST_FRA_SHIFT(dpsi, iv),
             ...
             CHF_CONST_FRA_SHIFT(thisBCoef, iv),
             CHF_CONST_FRA_SHIFT(thisCCoef, iv), CHF_BOX_SHIFT(region, iv),
             CHF_CONST_REAL(m_dx));
    }
}
...
void VariableCoeffPoissonOperator::setCoefs(
    ...
    const RefCountedPtr<LevelData<FArrayBox>> &a_cCoef, const Real &a_alpha,
    const Real &a_beta)
{
        ...
        for (dit.begin(); dit.ok(); ++dit)
        {
            ...
            This_will_not_compile !
                ...
                (CHF_FRA(a_dpsi[dit]), CHF_CONST_FRA(a_rhs[dit]),
                 ...
                 CHF_CONST_FRA((*m_aCoef)[dit]), CHF_CONST_REAL(m_beta),
                 CHF_CONST_FRA((*m_bCoef)[dit]), CHF_CONST_FRA((*m_cCoef)[dit]),
                 CHF_CONST_FRA(m_lambda[dit]), CHF_CONST_INT(whichPass));
...
void VariableCoeffPoissonOperator::setTime(Real a_time)
{
    ...
    // if (!m_cCoefInterpolator.isNull() &&
    //     !m_cCoefInterpolator->dependsUponSolution())
    //     m_cCoefInterpolator->interpolate(*m_cCoef, a_time);
