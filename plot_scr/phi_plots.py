...
hdf5_filenames = [ "../hdf5/DWp_000036.2d.hdf5", "../hdf5/DWp_000061.2d.hdf5", "../hdf5/DWp_000070.2d.hdf5", "../hdf5/DWp_000100.2d.hdf5"]
...
def plot_NG(a_data, ax, number):
    ...
    ax.plot(x_values,y_values, linestyle = '-' ,color='y', linewidth = 2.5)
...
for i, filename in enumerate(hdf5_filenames):
    ...
    slc.set_cmap(field="field1", cmap="RdBu")

    slc.set_zlim("field1", -1.0, 1.0)
 ...
 #   colorbar.update_ticks()
   ...
   # ax_inset = inset_axes(grid[1].axes, width="30%", height="30%", loc='upper right')
   # small_region = ds.box([0.45, 0.45, 0.45], [0.55, 0.55, 0.55])
 #   inset_ds = small_region.to_frb((100, ''), 512)
# Plot zoomed-in data
    ...
    grid[2].axes.set_xlabel(r"$z$",  fontsize=F2)
    grid[0].axes.set_xlabel("")
    grid[1].axes.set_xlabel("")

    grid[0].axes.set_ylabel(r"$r$",  fontsize=F2)
    grid[2].axes.set_ylabel(r"$r$",  fontsize=F2)
    grid[1].axes.set_xlabel("")
    ...
    plot.cax.set_ylabel("$\phi$", rotation=0, fontsize=F2)
    plot.cax.set_xticks([])
    plot.cax.set_yticks([-1.0, -0.5, 0.0, 0.5, 1.0])

    time = ds.current_time
    grid[3].axes.set_xlabel(r"$z$",  fontsize=F2)
...
plt.savefig("../plots/prolate_phi_plots_v2.pdf", dpi=200, bbox_inches = "tight")
plt.close()
