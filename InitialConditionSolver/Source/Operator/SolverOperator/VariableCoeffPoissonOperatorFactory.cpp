                      ...
                      const Real &a_beta)
{
    ...
    opFactory->define(a_params.coarsestDomain, a_grids, a_params.refRatio,
                      a_params.coarsestDx, &ParseBC, a_alpha, a_aCoef,
                      a_beta, a_bCoef, a_cCoef);
                                             ...
                                             int a_depth, bool a_homoOnly)
{
    ...
    else
    {
        ...
        CoarseAverage averager_c(m_cCoef[ref]->getBoxes(), layout,
                                 cCoef->nComp(), coarsening);
    ...
    const MGLevelOp<LevelData<FArrayBox>> &a_operator, int a_coarseningFactor)
{
    ...
    if (a_coarseningFactor != 1)
    {
        ...
        CoarseAverage cellAverage_c(ccoefFine.disjointBoxLayout(),
                                    ccoefCoar.disjointBoxLayout(), 1,
                                    a_coarseningFactor);
        for (DataIterator dit = ccoefCoar.disjointBoxLayout().dataIterator();
             dit.ok(); ++dit)
